...
export class BpmnDiagramComponent implements AfterContentInit, OnDestroy, OnInit {
  ...
  _chart(d3: any, data: any) {
    ...
    nodeL.append("rect")
      // .attr("class", "rect")
      .attr("width", (d: any) => {
        if (d.depth == 1) return d.data.title.length * 4;
      ...
      .attr("x", (d: any) => {
        if (d.depth == 1) return -d.data.title.length * 3;
        else return -d.data.title.length * 7;
      })
      .attr('y', (d: any) => {
        if (d.depth == 1) return '-1.9em';
        else return '-1em'
      })
      .attr("rx", (d: any) => {
    ...
    nodeL.append("text")
      ...
      .attr('y', '15')
      .attr('dy', (d: any) => {
        if (d.depth == 1) return '-2.4em';
        else return '-1.2em'
      })
      // .attr("dy", "0.3em")
      .attr("dx", (d: any) => {
        if (d.depth == 1) return -d.data.title.length * 2;
