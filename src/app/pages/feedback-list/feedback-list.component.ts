...
export class FeedbackListComponent {
  ...
  constructor(
    ...
    if (queryParams['type']?.length > 0) {
      if (queryParams['id']?.length > 0) this.deepLinkId = queryParams['id'];
    ...
    this.utils.getMeUpdatedList.subscribe((event: any) => {
      if (event) this.getMeConversations();
    });
        ...
        if (res) {
          ...
          if (res?.data.length) {
            if (this.deepLinkId.length > 0) {
              this.selectedListItem = res.data.filter(
                (item: any) => item.id === this.deepLinkId
              )[0];
              let selectedIndex = res.data.findIndex(
                (item: any) => item.id === this.deepLinkId
              );
