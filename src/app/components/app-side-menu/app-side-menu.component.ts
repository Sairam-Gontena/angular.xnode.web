...
export class AppSideMenuComponent implements OnInit {
  ...
  isUserAccessableToAgent: boolean = false;
  ...
  prepareMenuBasedOnRoute(): void {
    ...
    if(entityName.hasOwnProperty('entity_name') && entityName.entity_name.toLowerCase().includes('xnode')){
      this.isUserAccessableToAgent = true; 
    }
    if (!this.isInProductContext) {
      this.sideMenuItems = [
        ...
        {
          label: 'Agents',
          icon: './assets/images/robot_2.svg',
          path: 'agent-playground/',
          visible : this.isUserAccessableToAgent
        },
        {
          label: 'Products',
        ...
        {
          label: 'Knowledge',
          icon: './assets/images/auto_transmission.svg',
          path: 'my-products',
          visible : true
        }
      ];
  ...
  onClickMenuItem(item: any, i: any): void {
    ...
    switch(item.label){
      ...
      case 'Resources' : {
        this.storageService.saveItem(StorageKeys.IS_NAVI_EXPANDED, true)
        this.messagingService.sendMessage({
          msgType: MessageTypes.GO_TO_SELECTE_TAB,
          msgData: 'Resources',
        });
      break;
      }
