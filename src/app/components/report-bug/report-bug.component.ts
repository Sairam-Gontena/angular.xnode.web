...
export class ReportBugComponent implements OnInit {
  ...
  sendBugReport(): void {
    // var feedFiles = new Promise<void>((resolve, reject) => { if (index === this.uploadedFileData.length - 1) resolve();
    //    });     feedFiles.then(() => {   });

    // this.uploadedFileData.forEach((element: any, index: any) => {
    //   files.push({
    //     'fileId': element?.id,
  ...
  fileUploadCall(formData: any, headers: any, lastIndex: boolean) {
    this.commonApi.post('file-azure/upload', formData, { headers }).then((res: any) => {
      if (res) {
        ...
        if (lastIndex == true) {
          console.log('lastIndex', lastIndex, this.uploadedFileData)
          this.sendBugReport();
        }
        let user_audit_body = {
          'method': 'POST',
