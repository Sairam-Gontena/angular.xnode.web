...
export class GeneralFeedbackComponent implements OnInit {
  ...
  async onFileDropped($event?: any) {
    ...
    responses.forEach((response) => {
      if (response?.fileId) {
        this.feedbackReportFiles.push(response);
      }
    });
        ...
        .then((res: any) => {
          if (res) {
            ...
            this.auditUtil.postAudit(
              'FILE_DROP_FILE_AZURE_UPLOAD_GENERAL_FEEDBACK',
              1,
              'SUCCESS',
              'user-audit',
              user_audit_body,
    ...
    this.userUtilsApi
      ...
      .then((res: any) => {
        if (!res?.data?.detail) {
          ...
          this.auditUtil.postAudit(
            'USER_FEEDBACK_SEND_GENERAL_FEEDBACK_REPORT',
            1,
            'SUCCESS',
            'user-audit',
            user_audit_body,
          ...
          this.utils.loadToaster({
            severity: 'success',
            summary: 'SUCCESS',
            detail: 'General feedback submitted successfully',
          });
          ...
          this.auditUtil.postAudit(
            'GENERAL_FEEDBACK_' + res?.data?.detail,
            1,
            'FAILURE',
            'user-audit'
          );
  ...
  routeToFeedbackList() {
    ...
    this.router.navigate(['/feedback-list'], {
      queryParams: { type: 'user-feedback' },
    });
  }
