  ...
  getProduct(): void {
    this.apiService.get('navi/get_entire_data/' + this.currentUser?.email + '/' + this.product_id).then((res: any) => {
      if (res) {
        ...
        this.auditUtil.postAudit('PRODUCT_ALERT_POPUP_PUBLISH_PRODUCT', 1, 'SUCCESS', 'user-audit', user_audit_body, this.email, this.product_id);
      } else {
        let user_audit_body = {
          'method': 'GET',
          'url': res?.request?.responseURL
        ...
        this.auditUtil.postAudit('PRODUCT_ALERT_POPUP_PUBLISH_PRODUCT', 1, 'FAILED', 'user-audit', user_audit_body, this.email, this.product_id);
  ...
  publishProduct() {
    ...
    this.apiService.publishApp(body).then((response: any) => {
      if (response) {
        ...
        this.auditUtil.postAudit('PUBLISH_APP_PRODUCT_ALERT_POPUP', 1, 'SUCCESS', 'user-audit', user_audit_body, this.email, this.product_id);
        this.utils.loadSpinner(false);
        ...
        this.auditUtil.postAudit('PUBLISH_APP_PRODUCT_ALERT_POPUP', 1, 'FAILED', 'user-audit', user_audit_body, this.email, this.product_id);
  ...
  generateSpec() {
    ...
    this.apiService.postApi(body, 'specs/generate').then((response: any) => {
      if (response) {
        ...
        this.auditUtil.postAudit('GENERATE_SPEC_PRODUCT_ALERT_POPUP', 1, 'SUCCESS', 'user-audit', user_audit_body, this.email, this.product_id);
        this.utils.loadSpinner(false);
        this.utils.loadToaster({ severity: 'success', summary: 'SUCCESS', detail: detail });
        ...
        this.auditUtil.postAudit('GENERATE_SPEC_PRODUCT_ALERT_POPUP', 1, 'FAILED', 'user-audit', user_audit_body, this.email, this.product_id);
  ...
  getPreviousCoversation(): void {
    ...
    this.apiService.get('navi/get_conversation/' + this.currentUser?.email + '/' + this.product_id).then((res: any) => {
      if (res.status === 200 && res.data) {
        ...
        this.persistConversaiton();
        this.auditUtil.postAudit('GENERATE_APP_FROM_PRODUCT_POPUP', 1, 'SUCCESS', 'user-audit');
      } else {
        this.utils.loadSpinner(false);
        this.auditUtil.postAudit('GENERATE_APP_FROM_PRODUCT_POPUP', 1, 'FAILURE', 'user-audit');
  ...
  getMeTotalAppsPublishedCount(): void {
    this.apiService.get('navi/total_apps_published/' + this.currentUser?.account_id).then((res: any) => {
      if (res && res.status === 200) {
        ...
        this.auditUtil.postAudit('TOTAL_APPS_PUBLISHED_TEMPLATE_BUILDER_PUBLISH_HEADER', 1, 'SUCCESS', 'user-audit', user_audit_body, this.email, this.product_id);
      } else {
        this.utils.loadSpinner(false);
        ...
        this.auditUtil.postAudit('TOTAL_APPS_PUBLISHED_TEMPLATE_BUILDER_PUBLISH_HEADER', 1, 'FAILED', 'user-audit', user_audit_body, this.email, this.product_id);
      }
    }).catch((err: any) => {
      let user_audit_body = {
        'method': 'GET',
  ...
  sendEmailNotificationToTheUser(): void {
    ...
    this.notifyApi.post('email/notify', body).then((res: any) => {
      if (res && res?.data?.detail) {
        ...
        this.auditUtil.postAudit('EMAIL_NOTIFY_TO_USER_TEMPLATE_BUILDER_PUBLISH_HEADER', 1, 'SUCCESS', 'user-audit', user_audit_body, this.email, this.product_id);
        this.utils.loadSpinner(false);
      }
    }).catch((err: any) => {
      let user_audit_body = {
      ...
      this.auditUtil.postAudit('EMAIL_NOTIFY_TO_USER_TEMPLATE_BUILDER_PUBLISH_HEADER', 1, 'FAILED', 'user-audit', user_audit_body, this.email, this.product_id);
      this.utils.loadSpinner(false);
      this.utils.loadToaster({ severity: 'error', summary: 'ERROR', detail: err });
    })
  }
